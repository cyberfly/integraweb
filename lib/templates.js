...
// URL for a service: detail pages live at /services/<slug>.html unless the
// service overrides it (e.g. AI Training points to the training page).
const serviceUrl = (item) => item.frontmatter.url || `/services/${item.slug}.html`;

// A single service card for the services overview page. Mirrors the original
// hand-written `.build` blocks, with a link through to the detail page.
export function generateServiceCard(item) {
  const fm = item.frontmatter;
  const url = serviceUrl(item);
  const scope = (fm.scope || []).join(" · ");
  const linkLabel = fm.linkLabel || "Learn more";
  return `
    <div class="build reveal">
      <div class="build-num">${fm.num || ""}</div>
      <div class="build-body">
        <span class="build-time">${fm.eyebrow || fm.title}</span>
        <h3><a href="${url}" class="build-title-link">${fm.headline}</a></h3>
        <p>${fm.summary}</p>
        ${fm.tagline ? `<span class="win">${fm.tagline}</span>` : ""}
        <p style="margin-top:16px"><a href="${url}" class="read-more" style="font-weight:700;color:var(--brand);text-decoration:none">${linkLabel} →</a></p>
      </div>
      <div class="prompt-card">
        <span class="label">Typical scope includes</span>
        ${scope}
      </div>
    </div>`;
}

export function generateServicesOverview(services, template) {
  const cards = services.map(generateServiceCard).join("\n");
  return template.replace("<%- serviceCards %>", cards);
}

export function generateServiceDetail(item, allServices = []) {
  const fm = item.frontmatter;

  const scope = (fm.scope || [])
    .map((s) => `<div class="check"><div class="mk">✓</div><span>${s}</span></div>`)
    .join("");
  ...
  // Other services to cross-link from the bottom of the page.
  const others = allServices
    .filter((s) => s.slug !== item.slug)
    .map((s) => `<a href="${serviceUrl(s)}" class="read-more">${s.frontmatter.eyebrow || s.frontmatter.title} →</a>`)
    .join("");
  ...
  <section class="page-hero">
    <div class="wrap" style="max-width:760px">
      <span class="eyebrow">${fm.eyebrow || "Our services"}</span>
      <h1>${fm.headline}</h1>
  ...
  <section class="pad">
    <div class="wrap" style="max-width:740px">
      ...
      <div class="prose reveal" style="font-family:var(--body);color:var(--ink-soft);line-height:1.8">
        ${item.content}
      </div>
    </div>
  </section>
    ...
    <div class="wrap">
      <div class="prep-grid">
        <div class="reveal">
          ...
          <h2>Typical scope of work.</h2>
          <p class="lede" style="margin-top:14px">Every engagement is tailored — here's the kind of work this service typically covers.</p>
        </div>
        <div class="checklist reveal">${scope}</div>
      </div>
    </div>
  </section>`
      : ""
  ...
  ${
    benefits
      ? `<section class="pad-tight" style="background:var(--paper-2);border-top:1px solid var(--line)">
    <div class="wrap">
      <span class="eyebrow reveal">Why it matters</span>
      <h2 class="reveal">What you get out of it.</h2>
      <div class="why-grid">${benefits}</div>
    </div>
  </section>`
      : ""
  }
  ...
  ${
    others
      ? `<section class="pad-tight alt-bg">
    <div class="wrap">
      <span class="eyebrow reveal">Explore more</span>
      <h2 class="reveal" style="margin-bottom:24px">Our other services.</h2>
      <div class="service-links">${others}</div>
    </div>
  </section>`
      : ""
  }
  ...
  <section class="final pad">
    <div class="wrap">
      <span class="eyebrow" style="color:rgba(255,255,255,.75)">Ready when you are</span>
      <h2>Let's talk about your ${(fm.eyebrow || fm.title).toLowerCase()}.</h2>
      <p>Book a free consultation and we'll show you exactly where this can make the biggest difference for your business.</p>
      <a href="/contact.html" class="btn btn-white">Book a Free Consultation →</a>
    </div>
  </section>
  `;
